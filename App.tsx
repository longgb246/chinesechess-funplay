...
import { 
  Player, GameState, Cell, Position, Piece, GameMode
} from './types.ts';
...
import { Trophy, RotateCcw, Swords, Bomb, MousePointer2, Flag, LayoutGrid, Gamepad2, ArrowLeft, Grid3X3, Undo2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#fdfcf0] flex flex-col items-center py-8 px-4 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start justify-center w-full max-w-5xl">
        ...
        <div className="flex flex-col items-center order-1 lg:order-2">
          ...
          <GameBoard cells={board} onCellClick={handleCellClick} selectedPos={selectedPos} gameState={gameState} bombableIndices={[]} />
        </div>
...
const PlayerCard = ({ player, isCurrent, onBoard, inHand, onSurrender, gameOver }: { player: Player, isCurrent: boolean, onBoard: number, inHand: number, onSurrender: () => void, gameOver: boolean }) => {
    ...
    <div className={`p-4 rounded-xl shadow-md border-l-8 transition-all flex flex-col gap-2 ${isCurrent ? (isRed ? 'bg-red-50 border-red-600 scale-105' : 'bg-slate-100 border-slate-800 scale-105') : 'bg-white border-transparent opacity-60'}`}>
      <div className="flex items-center justify-between">
        <h2 className={`font-bold text-sm flex items-center gap-2 ${isRed ? 'text-red-800' : 'text-slate-800'}`}>
          {isRed ? '红方' : '黑方'} {isCurrent && <span className="text-[8px] bg-white px-2 py-0.5 rounded-full border border-current animate-pulse">行动中</span>}
        </h2>
        {isCurrent && !gameOver && (
